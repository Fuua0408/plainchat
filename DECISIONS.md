...
## 2026-07(039): WEB検索の出典表示(②)— 設計確定
- **決定(保存)**: tool_invocations は result_text に生テキストをそのまま1カラム保存する。ツール固有の
  フォーマットへ合わせた構造化カラム・解析は保存時には行わない(025/既存方針「MCPツール呼び出し全般を
  汎用的に永続化」を継承。ツールごとの特別扱いを保存層に持ち込まない)
- **決定(表示時の解析)**: フロントエンド(public/js/app.js)に tool_name→パーサー のルックアップを持ち、
  ...
  Title/Description/URL/Relevance Score のカードとして整形表示する。未知のツールは raw text を
  そのまま(pre書式)表示にフォールバックする。この解析はAPI契約・DBスキーマに影響しない表示層の機能
- **決定(表示層の実装場所)**: 既存パターン(添付ファイルチップ等)に倣いフロントエンド側に置く。
  バックエンドAPIは result_text を生のまま返すのみで、searxng固有の解釈を持たない
- **決定(取得経路)**: 直近ターンはSSEの done イベントに tool_invocations 配列を含めて返し即時表示する。
  過去ターンの再表示は GET /api/conversations/:id/messages の拡張(message.tool_invocations。
  attachments と同型の join パターン)で賄う。整形・折りたたみ描画関数はこの両経路で共用する
- **決定(mcp-searxngの実際の出力形式・調査結果)**: searxng_web_search は検索結果ごとに
  ...
  JSON配列ではなく半構造化テキスト1本。PlainChat側の src/mcp/client.js の extractTextContent()
...
## 2026-07(041完了): フロント資産キャッシュの自動バージョニング
- **背景**: 035で発見した「css/js変更後、実機ブラウザが古い資産を掴んで反映されない」問題への
  恒久対策。これまでindex.htmlの手書き?v=NNNクエリで都度バージョンを上げていたが、更新忘れの
  事故が起きうる(039-fixのような修正のたびに人手で番号を上げる運用は脆い)
- **決定(方式)**: ファイル内容のハッシュ(sha256先頭を切り詰め)でバージョンクエリを自動生成する。
  mtimeは不採用: git pull等のデプロイ方法によっては内容が変わらなくてもmtimeだけ変わりうるため、
  内容が変わったときだけ確実にURLを変えられるハッシュの方が頑健
- **決定(対象)**: css/style.css, js/api.js, js/app.js の3ファイルのみ。manifest/iconsは対象外
- **決定(実装場所)**: index.htmlをexpress.staticの既定index配信から外し、専用ルートで都度
  テンプレート処理(css/js参照にハッシュ付きクエリを注入)して返す。index.html自体には
  Cache-Control: no-cache を明示する(これが無いとindex.html自体が古いままキャッシュされ、
  ハッシュ方式が機能しなくなるため)
- **決定(キャッシュの鮮度)**: ハッシュはmtimeベースの軽量無効化で管理する(毎リクエストで
  fs.statのみ実行、実ファイル読み込み+ハッシュ再計算はmtime変化時のみ)。開発中にフロント資産を
  編集した際、サーバー再起動なしでも次のリクエストから反映される
- **やらないこと(今回のスコープ外)**: 静的資産へのCache-Control長期キャッシュ(max-age)設定。
  ハッシュ方式によりそれを安全に導入できる土台はできるが、導入自体は別タスクとする

- **実装(041完了)**: src/assetVersion.js(mtimeチェック→変化時のみsha256再計算・Mapキャッシュ)を
  新規作成。src/index.js で express.static を { index: false } 化し、GET / ・GET /index.html
  専用ハンドラをexpress.staticより前に登録、index.htmlを毎リクエスト読み込んでcss/style.css・
  js/api.js・js/app.jsの参照を現在のハッシュへ書き換え、Cache-Control: no-cache を付与する形にした。
  public/index.htmlソースからは手書き?v=NNNを削除。実サーバーで検証: バージョン付きURL・no-cache
  ヘッダの付与、app.js/style.css変更時のハッシュ変化(再起動なしで反映)、無変更時のハッシュ安定、
  他の静的資産(manifest/icons)への影響なし、ログイン/チャット画面の遷移に問題ないこと、を確認した